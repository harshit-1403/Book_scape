...
def thirteen():
    ...
    try:
        ...
        WHERE book_authors IS NOT NULL 
          AND book_authors != '' 
          AND year_of_publication IS NOT NULL
        GROUP BY book_authors, year_of_publication
        HAVING COUNT(*) > 1
        ORDER BY year_of_publication DESC, book_count DESC;
        """
        cursor.execute(query)
        rows = cursor.fetchall()
        if rows:
            # Display the results in a DataFrame
            columns = ["Author", "Year of Publication", "Number of Books"]
            df = pd.DataFrame(rows, columns=columns)
            st.subheader("Books Published by the Same Author in the Same Year")
            st.dataframe(df)
        else:
            st.warning("No records found for authors who published multiple books in the same year.")
...
def eighteen():
    ...
    try:
        ...
        if rows:
            ...
            columns = ["Book Type", "Average Retail Price"]
            df = pd.DataFrame(rows, columns=columns)
            ...
            # fig, ax = plt.subplots(figsize=(6, 4))
            # sns.barplot(data=df, x="Book Type", y="Average Retail Price", ax=ax, palette="Set2")
            # ax.set_title("Average Retail Price Comparison")
            # ax.set_ylabel("Average Retail Price")
            # ax.set_xlabel("Book Type")
            # st.pyplot(fig)
        else:
            st.warning("No retail price data available.")
